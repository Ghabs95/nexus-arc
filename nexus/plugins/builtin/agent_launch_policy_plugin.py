        ...
        if continuation:
            if continuation_payload and continuation_payload.startswith("You are @"):
                prompt = (
                    ...
                    f"1. Use only the Nexus-provisioned workspace and branch for this issue.\n"
                    f"2. Check the issue body for **Target Branch** and the specified starting branch.\n"
                    f"3. Do not create, switch, or rebase onto other branches manually.\n"
                    f"4. If you are the first implementation step and the instructions explicitly require a sync "
                    f"from the starting branch, only sync from that named starting branch.\n"
                    f"5. Make your changes and commit with descriptive messages.\n"
                    f"6. Push only the already-assigned issue branch when the workflow calls for it.\n\n"
                    ...
                    f"❌ NEVER run `git checkout <branch>`, `git checkout main`, `git checkout develop`, "
                    f"`git checkout -b ...`, or `git switch <branch>` unless the launch instructions for the "
                    f"first step explicitly say to sync from the named starting branch\n"
                    f"✅ ONLY push to the dedicated branch already provisioned by Nexus for this issue\n"
