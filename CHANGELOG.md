...
- **Social Platform Adapter Layer** — Implemented a robust, idempotent, and secure mechanism for multi-platform content distribution. This includes concrete Python implementations of the `SocialPlatformAdapter` protocol for Discord, X (Twitter), LinkedIn, and Meta (Facebook/Instagram). Key features include:
    ...
    Documented in [ADR-085](docs/ADR-085-Social-Platform-Adapters.md) ([#123](https://github.com/ghabs-org/nexus-arc/issues/123)).
- **Social Media Marketing Workflow Design** — Designed a new workflow for automated social media content creation,
  ...
  controls ([#119](https://github.com/ghabs-org/nexus-arc/issues/119)).
- **WebSocket Agent State Streaming** — Implemented a WebSocket-based streaming mechanism to push agent state changes
  ...
  ADR-083 ([#117](https://github.com/ghabs-org/nexus-arc/issues/117)).
- **Telegram Live Workflow Watch (`/watch`)** — Added a `/watch` command to the Telegram bot that subscribes to live
  ...
  ADR-089 ([#106](https://github.com/ghabs-org/nexus-arc/issues/106)).
- **Universal Nexus Identity (UNI)** — Refactored `UserManager` to use a platform-agnostic identity system. Users are
  ...
  platforms ([#86](https://github.com/ghabs-org/nexus-arc/issues/86)).
...
- **Feature Registry & Ideation Dedup (Telegram Bot)** — Added `FeatureRegistryService` to the Telegram bot to track
  ...
  summaries ([#88](https://github.com/ghabs-org/nexus-arc/issues/88)).
- **Slack Integration** — Added first-class Slack support via `SlackEventHandlerPlugin` (EventBus subscriber for 7
  ...
  `pip install nexus-arc[slack]`) ([#82](https://github.com/ghabs-org/nexus-arc/issues/82)).
- **Configurable Storage Adapters** — Introduced a unified configuration mechanism for storage backends (File and
  ...
  or the `NEXUS_STORAGE_BACKEND` environment variable ([#65](https://github.com/ghabs-org/nexus-arc/issues/65)).
...
- **YAML Workflow Orchestration** — Introduced `YamlWorkflowLoader` for loading and validating workflow definitions from
  ...
  retry policies ([#62](https://github.com/ghabs-org/nexus-arc/issues/62)).
...
- **OpenClaw Control Plane Integration (Planning Slice)** — Produced a phased implementation plan for the Nexus ↔ OpenClaw integration. Covers four pillars: (1) `WorkflowNotificationPayload` v1 rich notification envelope dispatched by `nexus/adapters/notifications/openclaw.py`; (2) stable session affinity via `nexus::workflow:<id>` keys stored in `WorkflowState.metadata`; (3) reply-to-workflow routing via `POST /api/v1/bridge/openclaw/reply` (requires auth + replay protection before merge); (4) `PlanModeSummary` dataclass for plan-mode dry-run output. ADR documented in [ADR-086](docs/ADR-086-OpenClaw-Control-Plane-Integration.md). Phase 1 implementation tasks defined and prioritised. Compliance blockers: bridge endpoint auth + TLS enforcement required before Phase 2 ship ([#130](https://github.com/ghabs-org/nexus-arc/issues/130)).
- **Social Media Marketing Workflow Design** — Designed a new workflow for automated social media content creation, approval, and multi-platform publishing. This includes a technical design document ([docs/DESIGN-Social-Media-Marketing-Workflow.md](docs/DESIGN-Social-Media-Marketing-Workflow.md)), an Architecture Decision Record ([ADR-084](docs/ADR-084-Social-Media-Marketing-Workflow.md)), and a selectable workflow definition ([examples/workflows/social_media_marketing_workflow.yaml](examples/workflows/social_media_marketing_workflow.yaml)). The workflow reuses existing enterprise agents (`triage`, `designer`, `developer`, `reviewer`, `compliance`, `deployer`, `writer`, `finalizer`) and supports platform-agnostic campaign management with dry-run-first publishing controls ([#119](https://github.com/ghabs-org/nexus-arc/issues/119)).
- **WebSocket Agent State Streaming** — Implemented a WebSocket-based streaming mechanism to push agent state changes directly to the Live Visualizer. This replaces poll-on-refresh with instantaneous updates via Socket.IO, supporting `step_status_changed`, `workflow_completed`, and live `mermaid_diagram` updates as required by ADR-083 ([#117](https://github.com/ghabs-org/nexus-arc/issues/117)).
- **Telegram Live Workflow Watch (`/watch`)** — Added a `/watch` command to the Telegram bot that subscribes to live workflow events from the `/visualizer` Socket.IO namespace and relays them to chat in real time. Supports `step_status_changed`, `workflow_completed`, and optional `mermaid_diagram` digest events. Includes exponential-backoff reconnect, dedup/throttle controls, and backend-safe subscription persistence for both `filesystem` and `postgres` storage modes. Feature is gated behind `NEXUS_TELEGRAM_WATCH_ENABLED`. Documented in ADR-089 ([#106](https://github.com/ghabs-org/nexus-arc/issues/106)).
- **Universal Nexus Identity (UNI)** — Refactored `UserManager` to use a platform-agnostic identity system. Users are now assigned a unique `nexus_id` (UUID4) and can link multiple platform identities (Telegram, Discord, etc.) to a single profile. This enables seamless profile synchronization and task history tracking across all supported chat platforms ([#86](https://github.com/ghabs-org/nexus-arc/issues/86)).
...
- **Slack Integration** — Added first-class Slack support via `SlackEventHandlerPlugin` (EventBus subscriber for 7 workflow lifecycle events, mrkdwn formatting) and `SlackInteractivePlugin` (Socket Mode via `slack-bolt`; no public URL required). Adds `SlackNotificationChannel` adapter. Requires `slack-bolt>=1.18` optional dependency (`pip install nexus-arc[slack]`) ([#82](https://github.com/ghabs-org/nexus-arc/issues/82)).
- **Configurable Storage Adapters** — Introduced a unified configuration mechanism for storage backends (File and PostgreSQL) via `WorkflowStateEnginePlugin`. Users can now switch backends using the `storage_type` configuration key or the `NEXUS_STORAGE_BACKEND` environment variable ([#65](https://github.com/ghabs-org/nexus-arc/issues/65)).
...
- **YAML Workflow Orchestration** — Introduced `YamlWorkflowLoader` for loading and validating workflow definitions from YAML. This enables complex multi-step AI workflows with support for parallel execution, conditional branching, and retry policies ([#62](https://github.com/ghabs-org/nexus-arc/issues/62)).
...
issues [#62](https://github.com/ghabs-org/nexus-arc/issues/62), [#119](https://github.com/ghabs-org/nexus-arc/issues/119)_
